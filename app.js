...
import { getFirestore, collection, addDoc, query, orderBy, onSnapshot, serverTimestamp, where, doc, updateDoc, deleteDoc, arrayUnion, arrayRemove, getDoc, setDoc } from "https://www.gstatic.com/firebasejs/10.4.0/firebase-firestore.js";
...
// ==========================================
// UTILS THỜI GIAN & MÀU SẮC CHO THÔNG BÁO (BẢN FIX LỖI)
// ==========================================
function formatTimeAgo(timestamp) {
    // Thêm check an toàn nếu timestamp bị null (do mạng chậm hoặc data cũ)
...
function getAvatarColor(char) {
    if (!char) return '#1877f2'; // Tránh lỗi nếu tên lớp bị trống
...
// ... (Giữ nguyên các sự kiện click dropdown) ...
...
function loadCourses(userEmail) {
    ...
    onSnapshot(q, (snapshot) => {
        ...
        snapshot.forEach(docSnap => {
            ...
            // --- XỬ LÝ THÔNG BÁO AN TOÀN ---
            // Đề phòng lớp học chưa có tên hoặc mã
            ...
            // Render Course Card (Giữ nguyên như cũ)
        ...
        // Đẩy vào bảng thông báo
        if(notiList) {
            notiList.innerHTML = notiHtml || '<div style="padding:20px; text-align:center; color:var(--text-muted);">Không có thông báo nào</div>';
        }
        
        // Hiển thị số đếm màu đỏ
...
// UI DROPDOWN THÔNG BÁO
document.getElementById('notification-btn').addEventListener('click', (e) => {
    e.stopPropagation();
    const dropdown = document.getElementById('notification-dropdown');
    dropdown.style.display = dropdown.style.display === 'none' ? 'block' : 'none';
    
    // Tắt badge khi mở xem thông báo (đánh dấu đã đọc local)
...
onAuthStateChanged(auth, async (user) => {
    ...
    if (user) {
        ...
        const avatar = user.photoURL || `https://ui-avatars.com/api/?name=${user.displayName || 'User'}`;
    ...
    saveSettingsBtn.addEventListener('click', async () => {
        ...
        try {
            ...
            await setDoc(doc(db, "users", currentUser.uid), {
                displayName: newName, bio: newBio, email: currentUser.email, updatedAt: serverTimestamp()
            }, { merge: true });
...
// ==========================================
// 1. QUẢN LÝ KHÓA HỌC (THÊM THÔNG BÁO TỰ ĐỘNG)
// ==========================================
...
window.openCourse = function(courseId, courseTitle) {
    ...
    // Lắng nghe Thông báo của lớp (Leader Post)
    const qAnnounce = query(collection(db, "course_announcements"), where("courseId", "==", courseId));
    unsubscribeCourseAnnouncements = onSnapshot(qAnnounce, (snapshot) => {
        const list = document.getElementById('course-announcements-list');
        list.innerHTML = '';
        if (snapshot.empty) {
            list.innerHTML = `<div class="modern-card" style="padding: 20px; text-align: center; color: var(--text-muted);">Chưa có thông báo nào từ Leader.</div>`;
            return;
        }

        snapshot.forEach(docSnap => {
            const data = docSnap.data();
            const announceId = docSnap.id;
                ...
                <div class="modern-card post">
                    <div class="post-header">
                        <div style="display:flex; align-items:center; gap:10px;">
                            ...
                            <div>
                                <span class="post-author-name">${data.name} <span class="badge-leader" style="font-size:0.7rem; padding:2px 6px; margin-left:5px;">Leader</span></span>
                            </div>
                        </div>
                        ${delBtn}
                    </div>
                    <div class="post-content" style="margin-bottom:15px; font-weight: 500;">${data.text}</div>
                    <div class="comment-section" style="padding-top:10px; border-top:1px solid #eee;">
                        ${commentsHtml}
                        ${currentUser ? `
                        <div style="display:flex; gap:8px; margin-top:10px; align-items:center;">
                            ...
                            <input type="text" id="announce-cmt-input-${announceId}" style="margin:0; border-radius:20px; padding:10px 15px; flex:1; border:none; background:#f0f2f5;" placeholder="Viết bình luận..." onkeypress="if(event.key==='Enter') addAnnounceComment('${announceId}')">
                        </div>` : ''}
                    </div>
                </div>`;
        });
    ...
    const qSlides = query(collection(db, "course_slides"), where("courseId", "==", courseId));
    unsubscribeCourseSlides = onSnapshot(qSlides, (snapshot) => {
        const list = document.getElementById('course-slides-list');
        list.innerHTML = '';
        if (snapshot.empty) {
            list.innerHTML = `<div class="modern-card" style="padding: 20px; text-align: center; color: var(--text-muted); grid-column: span 2;"><p>Chưa có tài liệu nào.</p></div>`; return;
        }
        snapshot.forEach(d => {
            const slide = d.data();
            const canEdit = currentUser && currentUser.email === ADMIN_EMAIL; 
            const delBtn = canEdit ? `<button class="btn-danger w-100" style="margin-top:10px;" onclick="delSlide('${d.id}')">Xóa Slide</button>` : '';
            
            list.innerHTML += `
                <div class="modern-card" style="padding: 15px; display:flex; flex-direction:column; justify-content:space-between;">
                    <div><div style="font-weight: 600; margin-bottom: 5px;">${slide.title}</div></div>
                    <div><a href="${slide.url}" target="_blank" class="btn-primary w-100" style="justify-content:center;"><i class="fa-solid fa-download"></i> Tải xuống</a>${delBtn}</div>
                </div>`;
        });
    });
    ...
    unsubscribeCourseRecords = onSnapshot(qRec, (snapshot) => {
        ...
        list.innerHTML = '';
        if (snapshot.empty) {
            list.innerHTML = `<div class="modern-card" style="padding: 20px; text-align: center; color: var(--text-muted); grid-column: span 2;"><p>Chưa có video nào.</p></div>`; return;
        }
        snapshot.forEach(d => {
            const rec = d.data();
            const delBtn = (currentUser && currentUser.email === ADMIN_EMAIL) ? `<button class="btn-danger w-100" style="margin-top:10px;" onclick="delRecord('${d.id}')">Xóa Video</button>` : '';
            list.innerHTML += `
                <div class="modern-card" style="padding: 15px;">
                    <div style="font-weight: 600; margin-bottom: 10px;">${rec.title}</div>
                    <a href="${rec.url}" target="_blank" class="btn-primary w-100" style="justify-content:center;"><i class="fa-brands fa-youtube"></i> Xem Video</a>${delBtn}
                </div>`;
        });
    });
};
...
// ==========================================
// TÍNH NĂNG THÔNG BÁO LỚP HỌC (LEADER POST)
// ==========================================
document.getElementById('submit-announcement').addEventListener('click', async () => {
    ...
    await addDoc(collection(db, "course_announcements"), {
        courseId: currentCourseId, text: text, uid: currentUser.uid, name: currentUser.displayName,
        photoURL: currentUser.photoURL, comments: [], createdAt: serverTimestamp()
    });

    document.getElementById('announce-content').value = '';
    document.getElementById('submit-announcement').disabled = false;
});
...
window.addAnnounceComment = async (announceId) => {
    ...
    try {
        await updateDoc(doc(db, "course_announcements", announceId), {
            comments: arrayUnion({ uid: currentUser.uid, name: currentUser.displayName, photoURL: currentUser.photoURL || "", text: input.value.trim(), time: Date.now() })
        });
        input.value = '';
...
document.getElementById('save-edit-course-btn').addEventListener('click', async () => {
    ...
    const newTitle = document.getElementById('edit-c-title').value.trim();
    const newCode = document.getElementById('edit-c-code').value.trim();
    const newDate = document.getElementById('edit-c-date').value.trim();
    const fileInput = document.getElementById('edit-c-image');
    ...
    try {
        if (fileInput.files.length > 0) {
            ...
            const sRef = ref(storage, 'course_images/' + Date.now() + '_' + file.name);
            await uploadBytes(sRef, file);
            updateData.imageUrl = await getDownloadURL(sRef);
        }
        await updateDoc(doc(db, "courses", currentCourseId), updateData);
        alert("Cập nhật thông tin khóa học thành công!");
        document.getElementById('edit-course-modal').style.display = 'none';
    } catch (error) { alert("Có lỗi xảy ra, vui lòng thử lại!");
    } finally { saveBtn.disabled = false; saveBtn.textContent = "Lưu thay đổi"; fileInput.value = ''; }
});
...
function renderStudentTable(courseId, emails, leaders, canEdit) {
    ...
    emails.forEach((email, index) => {
        ...
        let actionTd = '';
        if (canEdit) {
            const actionBtn = isLd ? `<button class="btn-icon" title="Hủy Leader" onclick="toggleLeader('${courseId}','${email}', false)"><i class="fa-solid fa-arrow-down" style="color:var(--warning)"></i></button>`
            ...
            actionTd = `<td class="no-print text-center">${actionBtn}<button class="btn-icon" title="Đuổi khỏi lớp" onclick="kickStudent('${courseId}','${email}')"><i class="fa-solid fa-user-minus" style="color:var(--danger)"></i></button></td>`;
        }
        tbody.innerHTML += `<tr><td class="text-center">${index + 1}</td><td><strong>${email}</strong></td><td>${roleBadge}</td>${actionTd}</tr>`;
    });
}
...
window.toggleLeader = async (cId, email, makeLeader) => { const refDoc = doc(db, "courses", cId); if(makeLeader) await updateDoc(refDoc, { leaders: arrayUnion(email) }); else await updateDoc(refDoc, { leaders: arrayRemove(email) }); };

// ==========================================
// UPLOAD SLIDE & VIDEO (Các phần giữ nguyên)
// ==========================================
const slideFileInput = document.getElementById('slide-file-input');
if(slideFileInput) { slideFileInput.addEventListener('change', (e) => { if(e.target.files.length > 0) { document.getElementById('slide-file-name').innerText = "Đã chọn: " + e.target.files[0].name; }});}
...
document.getElementById('submit-slide').addEventListener('click', async () => {
    ...
    const file = fileInput.files[0];
    const sRef = ref(storage, 'course_slides/' + Date.now() + '_' + file.name);
    alert("Đang tải file slide lên hệ thống...");
    await uploadBytes(sRef, file);
...
document.getElementById('send-course-chat').addEventListener('click', async () => {
    ...
    if (fileInput.files.length > 0) {
        status.style.display = 'block'; const file = fileInput.files[0]; fileName = file.name;
        const sRef = ref(storage, 'course_files/' + Date.now() + '_' + file.name);
        await uploadBytes(sRef, file); fileUrl = await getDownloadURL(sRef);
    }
...
document.getElementById('submit-record').addEventListener('click', async () => {
    ...
    await addDoc(collection(db, "course_records"), { courseId: currentCourseId, title, url, createdAt: serverTimestamp() });
...
// ==========================================
// BẢNG TIN TOÀN CẦU (POSTS, LIKES, COMMENTS)
// ==========================================
...
submitPostBtn.addEventListener('click', async () => {
    ...
    await addDoc(collection(db, "posts"), { text, fileUrl, fileName, uid: currentUser.uid, name: currentUser.displayName, photoURL: currentUser.photoURL, likes: [], comments: [], createdAt: serverTimestamp() });
    document.getElementById('post-content').value = ''; fileInput.value = ''; uploadStatus.style.display = 'none'; submitPostBtn.disabled = false;
});
...
document.getElementById('send-global-chat').addEventListener('click', async () => {
    ...
    await addDoc(collection(db, "chats"), { text: chatMsg.value, uid: currentUser.uid, name: currentUser.displayName, photoURL: currentUser.photoURL, createdAt: serverTimestamp() });
    chatMsg.value = '';
});
...
onSnapshot(query(collection(db, "chats"), orderBy("createdAt", "asc")), (snapshot) => {
    ...
    snapshot.forEach(docSnap => {
        ...
        const avatar = data.photoURL || `https://ui-avatars.com/api/?name=${data.name}&background=random`;
        chatBox.innerHTML += `<div class="chat-message ${alignClass}">${!isMe ? `<div class="chat-author">${data.name}</div>` : ''}<div style="display: flex; gap: 8px; align-items: flex-end; ${isMe ? 'flex-direction: row-reverse;' : ''}"><img src="${avatar}" class="avatar-sm" style="width: 28px; height: 28px; object-fit: cover;"><div class="chat-bubble">${data.text}</div></div></div>`;
    });
